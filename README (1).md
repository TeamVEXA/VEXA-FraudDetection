...
- Explains **WHY** it is flagged — not just yes/no *(Explainable AI)*
...
| Feature | Description |
| :--- | :--- |
| 🔍 Explainable AI | Reasons explain every fraud decision in plain English |
...
| ML Backend + API | Python 3.11 + Flask |
| Fraud Scoring | scikit-learn + predictor logic |
...
│   ├── predictor.py        ← ML fraud scoring + reasons
...
Java Spring Boot (validates + queues)
       ↓
Python Flask + scikit-learn (scores fraud probability)
       ↓
Predictor (generates "Why" explanation)
       ↓
